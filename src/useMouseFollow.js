 ...
 */
const useMouseFollow = (elementRef, { boundaryX, boundaryY, duration = 3000, follow = true } = {}) => {
    // Ref to store the current animation frame ID for cleanup purposes
    const animationFrameRef = (0, react_1.useRef)(null);
    (0, react_1.useEffect)(() => {
        const element = elementRef.current;
        if (!element)
            return; // Early exit if the element is not available
        /**
        ...
        const handleMouseMove = ({ clientX, clientY }) => {
            ...
            animationFrameRef.current = requestAnimationFrame(() => {
                ...
                const boundary = {
                    left: left + (elementWidth - (boundaryX !== null && boundaryX !== void 0 ? boundaryX : elementWidth)) / 2, // Left boundary
                    right: left + (elementWidth + (boundaryX !== null && boundaryX !== void 0 ? boundaryX : elementWidth)) / 2, // Right boundary
                    top: top + (elementHeight - (boundaryY !== null && boundaryY !== void 0 ? boundaryY : elementHeight)) / 2, // Top boundary
                    bottom: top + (elementHeight + (boundaryY !== null && boundaryY !== void 0 ? boundaryY : elementHeight)) / 2, // Bottom boundary
                };
                // Check if the mouse is within the defined boundary area
                const isWithinBoundary = clientX >= boundary.left &&
                ...
                if (isWithinBoundary) {
                    ...
                    const offsetX = clientX - (boundary.left + (boundaryX !== null && boundaryX !== void 0 ? boundaryX : elementWidth) / 2);
                    const offsetY = clientY - (boundary.top + (boundaryY !== null && boundaryY !== void 0 ? boundaryY : elementHeight) / 2);
                    // Apply smooth transformation to the element
                    element.style.transition = 'transform 0.1s ease-out'; // Fast follow animation
                    element.style.transform = `translate(${follow ? offsetX : -offsetX}px, ${follow ? offsetY : -offsetY}px)`;
                }
                else {
...
exports.default = useMouseFollow;
